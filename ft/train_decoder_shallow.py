...
run_name = f"[FT][S] {args.model_name.split('/')[1]}{'[QT]' if args.use_quantization else ''}"
wandb.init(
...
for run in range(args.runs):
    ...
    class DepressionClassifier(nn.Module):
        def __init__(self):
            ...
            self.fc1 = nn.Linear(model.config.hidden_size, 8)

        def forward(self, feature_vector):
            return self.fc1(feature_vector)
    ...
    # ---- Evaluation
    if args.use_peft and best_model_state:
        model.load_state_dict(best_model_state)

    if best_cls_head_state:
...
save_name = f"{args.model_name.split('/')[1]}_{'peft' if args.use_peft else ''}_shallow"
